...
def get_data(paths_to_data, filename, names_zbins=['0.50z0.85', '0.85z1.30'], identifier='W2', sigma_int=[0.25], pixel_scale=0.15, nzbins=1, mode='data', min_num_elements_pix=1., column_names={}):    
    ...
    for index_zbin in xrange(nzbins):
        ...
        g1, g2, shot_noise, field_properties, x, y, mask = reduce_data(tbdata, cols, x_coords[index_zbin], y_coords[index_zbin], xmin, xmax, ymin, ymax, identifier=identifier, sigma_int=sigma_int[index_zbin], pixel_scale=pixel_scale, fname_control=fname_binned, min_num_elements_pix=min_num_elements_pix, index_zbin=index_zbin, column_names=column_names)
    ...
    for index_zbin in xrange(nzbins):
        ...
        shot_noise_masked.append(shot_noise_all_zbins[index_zbin][maximal_mask.flatten()])
               
    return np.asarray(data_masked), shot_noise_masked, field_props_all_zbins, np.asarray(x_masked), np.asarray(y_masked)
    
def reduce_data(tbdata, cols, x_rad, y_rad, xmin, xmax, ymin, ymax, identifier='W2', sigma_int=0.25, pixel_scale=0.15, fname_control='your_name_here', min_num_elements_pix=1., index_zbin=1, column_names={}):
    ...
    g1_avg, bins_x, bins_y = np.histogram2d(x_rad, y_rad, bins=[bins_x, bins_y], weights=g1) 
    g1_avg = g1_avg.T / sum_weight / (1. + m1_bias_avg)
    
    g2_avg, bins_x, bins_y = np.histogram2d(x_rad, y_rad, bins=[bins_x, bins_y], weights=g2) 
    g2_avg = g2_avg.T / sum_weight / (1. + m2_bias_avg)
    ...
    return g1_avg.flatten(), g2_avg.flatten(), shot_noise, field_properties, xx.flatten(), yy.flatten(), mask
